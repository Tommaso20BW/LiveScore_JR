...
class CanvaExportTests(unittest.TestCase):
    ...
    def test_consecutive_requests_download_changed_page_one_and_identical_pdf(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            root = Path(root)
            folders = [canva.export_page_one(session, 'token', 'design', root,
                                            sleep=lambda _: None) for _ in range(3)]
            self.assertNotEqual(folders[0], folders[1])
            self.assertEqual(folders[1], folders[2])
            for folder, color, pdf in zip(folders, [(255, 0, 0), (0, 0, 255), (0, 0, 255)],
                ...
                with Image.open(folder / 'player.png') as player:
                    ...
                    for actual, expected in zip(player.getpixel((500, 700))[:3], color):
                        self.assertAlmostEqual(actual, expected, delta=3)
                self.assertEqual((folder / 'source.pdf').read_bytes(), pdf)
                self.assertEqual(json.loads((folder / 'manifest.json').read_text())['page'], 1)
            self.assertEqual(json.loads((root / 'current.json').read_text())['folder'], folders[-1].name)
        self.assertEqual(session.post.call_count, 3)
        for call in session.post.call_args_list:
            self.assertEqual(call.kwargs['json'], {'design_id': 'design',
                                                  'format': {'type': 'pdf', 'pages': [1]}})
        self.assertEqual([call.args[0] for call in session.get.call_args_list], [
            url for i in range(3) for url in (
    ...
    def test_failed_export_download_or_extraction_never_returns_old_layers(self):
        import pymupdf
        for stage in ('create', 'poll', 'failed_job', 'download', 'invalid_pdf', 'timeout'):
            with self.subTest(stage=stage), tempfile.TemporaryDirectory() as root:
                ...
                if stage == 'create':
                    ...
                    session.get.side_effect = [
                        Mock(json=lambda: {'job': {'status': 'success',
                    ...
                    if stage in ('failed_job', 'timeout'):
                        ...
                        session.get.return_value.json.return_value = {
                            'job': {'status': 'failed' if stage == 'failed_job' else 'inprogress'}}
                        expected = ValueError if stage == 'failed_job' else TimeoutError
                with self.assertRaises(expected):
                    canva.export_page_one(session, 'token', 'design', root, sleep=lambda _: None)
                self.assertEqual(json.loads((root / 'current.json').read_text())['folder'], old.name)
    ...
    def test_corrupt_cache_manifest_does_not_prevent_fresh_download(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            result = canva.export_page_one(export_session(pdf), 'token', 'design', root,
                                           sleep=lambda _: None)
            self.assertEqual((result / 'source.pdf').read_bytes(), pdf)
...
class PhaseFreshnessTests(unittest.TestCase):
    common = dict(data_espn={}, home_id='111', away_id='110', home_name='Juventus',
                  away_name='Inter', league_slug='ita.1', league_name='Serie A')
    ...
    def test_every_phase_request_including_repeated_phase_uses_new_layers(self):
        ...
        with patch.object(bot, 'GOAL_GRAPHICS_ENABLED', True), \
            ...
            for kind, folder in zip(('half', 'end_of_90', 'full', 'full'), folders):
                self.assertEqual(bot.build_phase_graphic(kind=kind, **self.common), b'png')
                self.assertEqual(render.call_args.kwargs['layers'], folder)
    ...
    def test_retry_after_export_failure_downloads_again_before_rendering(self):
        ...
        with patch.object(bot, 'GOAL_GRAPHICS_ENABLED', True), \
             patch.object(bot, 'get_valid_token', return_value='token'), \
             patch.object(bot, 'rileva_kit_juve', return_value='home'), \
             patch.object(canva, 'export_page_one', side_effect=[ValueError('failed'), fresh]) as export, \
            ...
            self.assertEqual(bot.send_phase_message('Half time', kind='half', **self.common), 42)
        self.assertEqual(export.call_count, 2)
        render.assert_called_once()
        self.assertEqual(render.call_args.kwargs['layers'], fresh)
        send.assert_called_once_with('Half time')
        edit.assert_called_once_with(42, 'Half time', b'new-png')

    def test_persistent_export_failure_leaves_text_and_never_renders_old_image(self):
        with patch.object(bot, 'GOAL_GRAPHICS_ENABLED', True), \
             patch.object(bot, 'get_valid_token', return_value='token'), \
             patch.object(canva, 'export_page_one', side_effect=ValueError('failed')) as export, \
            ...
            self.assertEqual(bot.send_phase_message('Full time', kind='full', **self.common), 42)
