def main():
    ...
    while True:
        try:
            ...
            # Chiamata LIVE standard
            params = {"team": JUVE_ID, "live": "all"}
            response = requests.get(url, headers=headers, params=params, timeout=15)
            response.raise_for_status()
            res = response.json()
            
            # SE LA PARTITA NON È PIÙ NEI FEED LIVE MA STAVAMO TRACCIANDO UNA PARTITA
            if (!res.get('response') or len(res['response']) == 0) and state.get("live_match_id") is not None:
                print("⚠️ Partita sparita dal feed Live. Verifico se è terminata...")
                # Facciamo una chiamata specifica per ID per recuperare il finale
                check_params = {"id": state["live_match_id"]}
                check_resp = requests.get(url, headers=headers, params=check_params, timeout=15).json()
                if check_resp.get('response') and len(check_resp['response']) > 0:
                    res['response'] = check_resp['response']
                else:
                    print("Impossibile recuperare i dettagli del match. Attendo...")
                    time.sleep(90)
                    continue
            
            # Se invece non c'è proprio nessuna partita e non ne stavamo tracciando una
            ...
            # Verifica se la partita è effettivamente finita
            is_finished = "finished" in status_long or status in ["FT", "AET", "PEN"] and "finished" in status_long
            
            if status == "PEN" and not is_finished:
                current_sleep_time = 60   
            elif status in ["ET", "AET"] and not is_finished:
            ...
            if state.get("live_match_id") != current_match_id and not is_finished:
                state = {
            ...
            # Se la partita è finita prima che registrassimo l'inizio, aggiorna l'ID di stato
            if state.get("live_match_id") is None:
                state["live_match_id"] = current_match_id
            ...
            elif status in ["ET", "AET", "PEN"] and "2H_END" not in state["sent_periods"] and not is_finished:
                msg = f"<b>FINE SECONDO TEMPO {E_FLAG}</b>\n\n{home_emoji} {home_name} {g_home_int}-{g_away_int} {away_name} {away_emoji}\n\n{e_comp} {hashtag}"
                send_telegram(msg)
                state["sent_periods"].append("2H_END")

            if status == "ET" and not is_finished:
            ...
            # --------------------------------------------------------------------------
            # 2. AVVISI GOL LIVE E ANNULLAMENTO VAR (Solo se la gara è in corso)
            # --------------------------------------------------------------------------
            if not is_finished:
            ...
            # --------------------------------------------------------------------------
            # 3. GESTIONE EVENTI SPECIALI (Solo se la gara è in corso)
            # --------------------------------------------------------------------------
            if not is_finished:
                events = match.get('events', [])
                if events:
            ...
            # Salva lo stato ad ogni ciclo se non è finita
