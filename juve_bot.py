...
def build_scorers_text(events):
    ...
    for e in events:
        if e.get('type', '').lower() == 'goal':
            ...
            # Controllo rigori o autogol nelle stringhe dell'API
...
def main():
    # Caricamento persistenza dello stato per evitare duplicati
    ...
    try:
        ...
        print(f"Errore chiamata HTTP API: {e}")
        return

    # Se non c'è una partita in corso, resetta lo stato ed esce
    if not res.get('response') or len(res['response']) == 0:
        print("Nessuna partita live trovata per la Juventus.")
        state = {"live_match_id": None, "sent_periods": [], "goals_detected": 0, "sent_subs": [], "sent_cards": []}
    ...
    # Se è una nuova partita, pulisci lo stato precedente
    ...
    # --------------------------------------------------------------------------
    # 1. BLOCCHI CRONACA PERIODI DI GARA (Inizio, Intervallo, Fine)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # 2. AVVISI GOL LIVE (In tempo reale)
    # --------------------------------------------------------------------------
    ...
    for e in events:
        ...
        # --- SOSTITUZIONI (Filtro automatico solo per la Juventus) ---
        ...
        # --- CARTELLINI (Mappatura icone Premium per Gialli e Rossi) ---
        elif ev_type == 'card':
    ...
    # Invio dei cambi raggruppati (Evita post separati se ci sono cambi doppi al medesimo minuto)
