...
def avvia_ciclo_partita():
    ...
    while True:
        ...
        try:
            ...
            # --- Catchup eventi pregressi: bot avviato a partita già in corso ---
            if state["goals_detected"] == 0 and (g_home + g_away) > 0 and not state.get("goal_messages"):
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ℹ️  Partita già in corso ({g_home}-{g_away}), invio catchup eventi pregressi")

                # Messaggio di stato iniziale
                send_telegram(
                    f"<b>PARTITA IN CORSO {E_BOLT}</b>\n\n"
                    f"{score_str}\n"
                    f"⏱ {elapsed}'\n\n"
                    f"{e_comp} {hashtag}"
                )
                ...
                # Ricostruisci il punteggio progressivo per ogni goal
                sim_h, sim_a = 0, 0
                for ge in goal_events:
                    ...
                    assist_name = ge.get("assist_name", "")

                    if player_name:
                        ps = fmt_player(player_name)
                        if ge["type"] == "own goal":
                            ps += " (Autogol)"
                        elif ge["type"] == "penalty goal":
                            ps += " (Rig.)"
                    ...
                    if actual_tid == home_id:
                        goal_score = f"<b>{home_name} {sim_h}</b>-{sim_a} {away_name}"
                    else:
                        goal_score = f"{home_name} {sim_h}-<b>{sim_a} {away_name}</b>"

                    goal_text = (
                        f"<b>GOAL · {ge['minute']}' {E_MIC}</b>\n\n"
                        f"{goal_score}\n"
                        f"{scorer_line}{assist_line}\n"
                        f"{e_comp} {hashtag}"
                    )
                    goal_key = f"{sim_h}_{sim_a}"
                    _log = fmt_player(player_name) if player_name else "(marcatore sconosciuto)"
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏪ CATCHUP GOAL {ge['minute']}' {_log} ({home_name} {sim_h}-{sim_a} {away_name})")
                ...
                for e in events:
                    if e["type"] in ("red card", "second yellow card"):
                        ...
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏪ CATCHUP ROSSO {e['minute']}' {p_name}")
                        send_telegram(
                ...
                for e in events:
                    if e["type"] in ("penalty missed", "penalty saved"):
                        ...
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⏪ CATCHUP RIGORE SBAGLIATO {team_name} {e['minute']}' {fmt_player(e['player_name'])}")
                        send_telegram(
            ...
            for goal_key, saved in list(state.get("goal_messages", {}).items()):
                ...
                if (current_scorer != saved.get("scorer")) or \
                   ...
                   (current_type != saved.get("type", "goal")):
                    ...
                    # Mantieni la squadra originariamente associata al goal.
                    # Se ESPN corregge goal -> autogol cambiamo solo testo,
                    # marcatore e assist, non la squadra evidenziata.
                    
                    actual_tid = saved.get("score_tid", s_tid)
