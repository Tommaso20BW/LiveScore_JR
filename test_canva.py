...
def update_github_secret(secret_name, new_value):
    ...
    if not github_pat or not repo:
        print("⚠️ Avviso: Credenziali GitHub non trovate in ambiente. Salto l'auto-aggiornamento del Secret.")
        return
    ...
    try:
        ...
        if res_update.status_code in [201, 204]:
            print(f"🔄 [AUTO-REFRESH] Secret '{secret_name}' aggiornato su GitHub per la prossima partita!")
        else:
...
def get_canva_image(access_token):
    ...
    try:
        ...
        print("⏳ Attesa del rendering trasparente su Canva...")
        for i in range(20):
            time.sleep(3)
            ...
            if check_res.status_code == 200:
                ...
                if status_corrente == "success":
                    ...
                    if download_url:
                        print("📥 Download del livello PNG trasparente completato.")
...
# ==============================================================================
# CORE: FILTRI EVENTO E SOVRAPPOSIZIONE IMMAIGINI (1:1 COINCIDENTE)
# ==============================================================================
