...
# ==============================================================================
# GESTIONE DOPPIA API KEY CON FALLBACK AUTOMATICO
# ==============================================================================
API_KEYS = [
    k for k in [os.getenv('API_KEY_1'), os.getenv('API_KEY_2')]
    if k  # esclude le chiavi non configurate
]
active_key_index = 0  # parte sempre con la Key 1

def get_api_headers():
    """Restituisce sempre gli headers con la chiave attualmente attiva."""
    return {"x-apisports-key": API_KEYS[active_key_index]}
...
def update_github_secret(secret_name, new_value):
    if not GH_PAT or not GITHUB_REPOSITORY:
        print("⚠️ Impossibile aggiornare il secret: GH_PAT o GITHUB_REPOSITORY non presenti.")
        return False
    headers = {
    ...
    try:
        ...
        if res_secret.status_code in [201, 204]:
            ...
            print(f"❌ Errore aggiornamento secret GitHub: {res_secret.text}")
            return False
    except Exception as e:
        print(f"❌ Eccezione aggiornamento secret GitHub: {e}")
        return False
...
def get_valid_token():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"❌ Errore token Canva: {res.text}")
            return None
    except Exception as e:
...
# ==============================================================================
def recupera_e_genera_stats_html(match_id, home_id, away_id, home_name, away_name, home_goals, away_goals, momento, league_name="SERIE A"):
    print(f"📊 Recupero statistiche per il momento {momento}...")
    ...
    api_stats = {
        "Shots on Goal": [0, 0], "Total Shots": [0, 0], "Fouls": [0, 0],
        "Corner Kicks": [0, 0], "Ball Possession": ["50%", "50%"],
        "Yellow Cards": [0, 0], "Red Cards": [0, 0],
        "expected_goals": ["0.0", "0.0"], "Passes accurate": [0, 0]
    }

    try:
        # Questa chiamata usa anch'essa api_request con fallback automatico
        res = api_request(stats_url)
        if res and res.get('response') and len(res['response']) >= 2:
        ...
        print(f"⚠️ Errore statistiche: {e}. Uso valori di fallback.")
...
<div class="card">
  ...
  <div class="stats-body">
    ...
    print("📸 Rendering con Playwright (1620x1980)...")
    ...
    if os.path.exists("texture.png"):
        try:
            base    = Image.open(path_raw_png).convert("RGBA")
            texture = Image.open("texture.png").convert("RGBA")
            texture = texture.resize(base.size, Image.Resampling.LANCZOS)
            out     = Image.alpha_composite(base, texture)
            out.convert("RGB").save(path_final_png, "PNG")
...
# ==============================================================================
# CICLO PRINCIPALE LIVE
# ==============================================================================
def avvia_ciclo_partita():
    print("✅ Procedo al recupero del match...")

    if not API_KEYS:
        print("Errore: nessuna API_KEY configurata.")
        return
    ...
    while not match_id:
        ...
        print(f"🔄 [Controllo Palinsesto] Cerco partita Juventus ({today_date})... [Key attiva: {active_key_index + 1}]")

        try:
            live_res = api_request(f"{url}?live=all")
            if live_res and live_res.get('response'):
            ...
            if not match_id:
                date_res = api_request(url, params={"team": JUVE_ID, "date": today_date})
                if date_res and date_res.get('response') and len(date_res['response']) > 0:
                    match_id = date_res['response'][0]['fixture']['id']
                    print(f"📅 Match trovato nel palinsesto di oggi! ID: {match_id}")

            if not match_id:
                next_res = api_request(url, params={"team": JUVE_ID, "next": 1})
                if next_res and next_res.get('response') and len(next_res['response']) > 0:
                    match_data = next_res['response'][0]
                    match_id = match_data['fixture']['id']
                    print(f"📌 Prossimo match in calendario. ID: {match_id} ({match_data['fixture']['date']})")

        except Exception as e:
            print(f"⚠️ Errore temporaneo nel recupero dati: {e}")

        if not match_id:
            print("❌ Nessun match trovato. Riprovo tra 30 secondi...")
            time.sleep(30)

    print(f"⏳ Bot agganciato all'ID {match_id}. Entro nel ciclo di monitoraggio...")
        ...
        try:
            if os.path.exists("match_state.json"):
                ...
                state = {
                    ...
                    "sent_subs": [], "sent_cards": [], "sent_failed_penalties": [],
                    "penalties_count": 0, "sent_stats": []
                }

            res = api_request(url, params={"id": match_id})

            if not res or not res.get('response') or len(res['response']) == 0:
                print(f"⚠️ Nessun dato ricevuto dall'API. Riprovo tra 30 secondi... [Key attiva: {active_key_index + 1}]")
                time.sleep(30)
                continue
            ...
            if status in ["NS", "TBD"] and g_home_int == 0 and g_away_int == 0 and elapsed_minutes == 0:
                print(f"💤 Match non ancora iniziato (Stato: {status}). Riprovo tra 30 secondi...")
                time.sleep(30)
                continue
            ...
            score_string = (
                f"{g_home_int} ({p_home}) - ({p_away}) {g_away_int}"
                if p_home is not None else f"{g_home_int}-{g_away_int}"
            )
            ...
            print(f"[LIVE] {home_name} {score_string} {away_name} | Min: {elapsed_minutes} | Key attiva: {active_key_index + 1}")
            ...
            # 2. FINE PRIMO TEMPO
                ...
                print("⏳ Attesa 2 minuti consolidamento dati HT...")
                time.sleep(120)
                png_path = recupera_e_genera_stats_html(match_id, home_id, away_id, home_name, away_name, g_home_int, g_away_int, "HT", league_name)
            ...
            # 4. FINE REGOLAMENTARI (si va ai supplementari)
                ...
                print("⏳ Attesa 2 minuti consolidamento dati 2H_END...")
                time.sleep(120)
                png_path = recupera_e_genera_stats_html(match_id, home_id, away_id, home_name, away_name, g_home_int, g_away_int, "2H_END", league_name)
            ...
            if status in ["FT", "AET"] or (status == "PEN" and status_long == "match finished"):
                print("🏁 FISCHIO FINALE! Connessione a Canva...")
                ...
                print("⏳ Attesa 2 minuti consolidamento dati FT...")
                time.sleep(120)
                png_path = recupera_e_genera_stats_html(match_id, home_id, away_id, home_name, away_name, g_home_int, g_away_int, "FT", league_name)
                ...
                print("🏁 Processo terminato. Spegnimento bot.")
                sys.exit(0)
            ...
            if events:
                ...
                for sub_key, sub_data in subs_by_minute.items():
                    team_title = (
                        "JUVENTUS" if sub_data["team_id"] == JUVE_ID else
                        home_name.upper() if sub_data["team_id"] == home_id else
                        away_name.upper()
                    )
