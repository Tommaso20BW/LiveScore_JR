...
def main():
    ...
    # Rimosso "live": "all" per evitare che la partita sparisca a fine match.
    # Cerchiamo la partita della Juve in data odierna.
    ...
    while True:
        try:
            ...
            if not res.get('response') or len(res['response']) == 0:
                print("In attesa che la partita della Juventus inizi o sia disponibile per oggi...")
                time.sleep(90)
                continue
            ...
            # Se la partita non è ancora iniziata (Not Started)
            if status == "NS":
                print("La partita è in programma oggi ma non è ancora iniziata.")
                time.sleep(90)
                continue
            ...
            h_short = "Juve" if home_id == JUVE_ID else home_name
            a_short = "Juve" if away_id == JUVE_ID else away_name
            ...
            # ==============================================================================
            # SPEGNIMENTO AUTOMATICO A FINE PARTITA (LOGICA CORRETTA)
            # ==============================================================================
            status_long = fixture.get('status', {}).get('long', '').lower()
            
            # Semplificato e reso sicuro il controllo di fine match
            ...
            if is_finished and "FT" not in state["sent_periods"]:
                ...
                # Salvataggio finale prima di cancellare
