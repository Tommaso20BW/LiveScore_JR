...
# ⚙️ TEST: forza il bot a cercare una squadra diversa ma trattarla come TEAM_ID
# Es: OVERRIDE_TEAM_ID=13 → cerca Inter Milan ma monitora come se fosse la Juve (111)
# Lascia vuoto ('') per comportamento normale
OVERRIDE_TEAM_ID    = os.getenv('OVERRIDE_TEAM_ID', '')
...
def trova_partita_oggi(team_id: str):
    # ⚙️ TEST MODE: se OVERRIDE_TEAM_ID è impostato, cerca quella squadra nell'API
    # ma poi sostituisce il suo ID con team_id (es. Juventus 111) nei competitors
    search_id = OVERRIDE_TEAM_ID.strip() if OVERRIDE_TEAM_ID.strip() else team_id
    if search_id != team_id:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚙️  TEST MODE: cerco team_id={search_id} (override), tratterò come team_id={team_id}")
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔍 Cerco partita per team_id={search_id}...")
        ...
        for slug in LEAGUE_SLUGS:
            ...
            try:
                ...
                for event in data.get("events", []):
                    ...
                    if search_id in ids:
                        # ⚙️ TEST MODE: sostituisce l'ID della squadra trovata con team_id
                        if search_id != team_id:
                            for c in competitors:
                                if c.get("team", {}).get("id", "") == search_id:
                                    c["team"]["id"] = team_id
                                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚙️  ID {search_id} → {team_id} nei competitors")
